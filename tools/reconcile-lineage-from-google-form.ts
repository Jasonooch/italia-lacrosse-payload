...
if (!csvArg) {
  console.error('Usage: tsx tools/reconcile-lineage-from-google-form.ts <google-form-export.csv> [--dry-run]')
  process.exit(1)
}
...
const pendingUpdates: Array<{
  ...
  nextLineage: LineageValue
  currentCitizenship: string
  nextCitizenship: 'citizen' | 'pending' | 'not-a-citizen'
}> = []
...
for (const [email, source] of lineageByEmail.entries()) {
  ...
  pendingUpdates.push({
    email,
    currentLineage: contact.lineage || '',
    nextLineage: source.lineage,
    currentCitizenship: contact.citizenship || '',
    nextCitizenship,
  })
  if (!dryRun) {
...
if (pendingUpdates.length > 0) {
  console.log('\nContacts with lineage/citizenship changes:')
  pendingUpdates.forEach((item) => {
    console.log(
      `- ${item.email}: lineage "${item.currentLineage}" -> "${item.nextLineage}", citizenship "${item.currentCitizenship}" -> "${item.nextCitizenship}"`,
    )
  })
}
